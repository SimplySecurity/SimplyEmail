...
class Conducter:
	...
	# 6) The module will place the results (emails) into a results queue in memmory so we can output to Sqlite or WebPage or Console
		...
		while True: 
			...
			try:
				...
				try:
					...
					if Emails:
						count = len(Emails)
						Length = "[*] " + Module.name + ": Gathered " + str(count) + " Email(s)!"
	...
	def TaskSelector(self, domain):
		...
		# How many proc will we have, pull from config file, setting up the config file handler
		...
		for thread in range(total_proc):
			procs.append(multiprocessing.Process(target=self.ExecuteModule, args=(Task_queue,Results_queue, domain)))
		for p in procs:
			p.daemon = True
			p.start()
		for p in procs: 
