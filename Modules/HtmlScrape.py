...
class ClassName:
		...
		def __init__(self, domain):
				...
				# Settings we will pull from config file (We need required options in config file)
				...
				try:
					...
					self.limit_rate = "--limit-rate=" + str(config['HtmlScrape']['LimitRate'])
					self.timeout = "--read-timeout=" + str(config['HtmlScrape']['Timeout'])
					self.save = "--directory-prefix=" + str(config['HtmlScrape']['Save'])
				except:
		...
		def get_emails(self):
			...
			try:
				...
				val = subprocess.check_output(("grep", "-i", "-o", '[A-Z0-9._%+-]\+@[A-Z0-9.-]\+\.[A-Z]\{2,4\}'),
																				stdin=ps.stdout)
				# Supper "hack" since the data returned is from Pipelin /n and all in var
